<?php
/**
 * Plugin Name:       GROK XAI AI Design Generator
...
class Ruslan_AI_Design_Generator {
    ...
    public function register_block() {
        register_block_type('ruslan-ai/generator', [
            'apiVersion'      => 3, // Виправлено deprecated warning
    ...
    public function render_block($attributes) {
        if (empty($attributes['generatedHtml'])) {
            return '<div class="ai-design-placeholder p-8 text-center border-2 border-dashed border-gray-300 rounded-3xl text-gray-400">GROK XAI AI Design Generator 4.0.2<br>Оберіть режим і натисніть "Виконати"</div>';
        }
    ...
    public function generate_design(WP_REST_Request $request) {
        ...
        if ($mode === 'edit_theme') {
            ...
            $system_prompt = "Ти — експерт WordPress Block Theme. Онови файл за промптом користувача. Повертай ТІЛЬКИ чистий оновлений код (без пояснень). Збережи Tailwind, адаптивність і dark mode.";
        ...
        // === SINGLE / FULL / THEME (повна логіка) ===
        $system_prompt = $mode === 'full' 
            ? "Ти — експерт UI/UX. Створи повноцінний HTML-шаблон сторінки ({$request->get_param('templateType')}) з Tailwind CSS. Додай скрипт Tailwind на початку. Зроби максимально сучасним і адаптивним."
            : ($mode === 'theme' 
                ? "Створи повну Block Theme для WordPress за описом. Повертай тільки JSON з файлами." 
                : "Створи один красивий HTML-блок з Tailwind CSS.");
        ...
        $body = json_decode(wp_remote_retrieve_body($response), true);
        $html = $body['choices'][0]['message']['content'] ?? 'Помилка генерації';

        return rest_ensure_response(['html' => trim($html)]);
    }
...
(function() {
    ...
    registerBlockType('ruslan-ai/generator', {
        title: __('GROK XAI AI Design Generator 4.0.2', 'ruslan-ai-design-generator'),
        icon: 'format-image',
        category: 'design',
        attributes: {
            prompt: { type: 'string', default: '' },
        ...
        edit: function(props) {
            ...
            return wp.element.createElement('div', useBlockProps(),
                wp.element.createElement(InspectorControls, {},
                    wp.element.createElement(PanelBody, { title: __('GROK XAI Генерація 4.0.2', 'ruslan-ai-design-generator'), initialOpen: true },
                        ...
                        wp.element.createElement(TextareaControl, {
                            ...
                            placeholder: 'Опишіть, що потрібно створити або змінити...',
                            __nextHasNoMarginBottom: true
                        }),
...
JS;
    ...
    public function add_settings_page() {
        add_options_page('GROK XAI AI', 'GROK XAI AI', 'manage_options', 'ruslan-ai-design-generator', [$this, 'settings_page_html']);
    }

    public function register_settings() {
        register_setting('ai_design_settings', 'ai_design_openai_key', 'sanitize_text_field');
        add_settings_section('ai_design_main', 'OpenAI API Key', null, 'ruslan-ai-design-generator');
    ...
    public function settings_page_html() {
        ?>
        <div class="wrap">
            <h1>GROK XAI AI Design Generator — Налаштування</h1>
            <form method="post" action="options.php">
                <?php
                settings_fields('ai_design_settings');
                do_settings_sections('ruslan-ai-design-generator');
                submit_button();
                ?>
            </form>
        </div>
